...
stat_test <- nasal_shedding %>% 
  wilcox_test(MX1 ~ n_shed, comparisons = my_comparisons) %>%
  add_xy_position(x = "n_shed") %>%
  mutate(myformatted.p = paste0("p = ", p))

lm_test <- lm(MX1 ~ as.numeric(n_shed), data = nasal_shedding)
summary(lm_test)
...
nasal_shedding %>%
  ggplot(aes(x = n_shed, y = MX1)) +
  geom_boxplot(outlier.shape = NA, colour = "darkblue") +
  geom_jitter(width = 0.1, size = 0.75, colour = "darkblue") +
  annotate("text", x = 1.75, y = 3, label = formatted_global_p, size = 4.5, hjust = 0) +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 12)) +
  xlab("Number of strains shed") +
  ylab(expression(italic(MX1) ~ "expression (" * Log[2] * " fold change)"))

ggsave("./output/figs/fig4c.svg")
ggsave("./output/figs/fig4c.png")
...
nasal_shedding %>%
  ...
  geom_boxplot(outlier.shape = NA, colour = "darkblue") +
  geom_jitter(width = 0.1, size = 0.75, colour = "darkblue") +
  annotate("text", x = 1.75, y = 5.5, label = formatted_global_p, size = 4, hjust = 0) +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 12)) +
  xlab("Number of strains shed") +
...
ggsave("./output/figs/fig4d.svg")
ggsave("./output/figs/fig4d.png")

####
## Fig 4e
####
baseline_serostatus <- read.csv("./data/metadata.csv", header = T) %>%
dplyr::select(subid1, contains("v0_gmt")) %>%
  dplyr::rename("subid" = subid1)

baseline_serostatus$n_prior_exposure <- rowSums(baseline_serostatus[, c("h1_v0_gmt", "h3_v0_gmt", "b_v0_gmt")] > 5)

nsl_hai_genes <- c("IRF8", "IRF4", "HERC5", "ZBP1", "DHX58", "MX1", "IFI6", "APOBEC3A", "IFIT1")
nasal_fc <- 
  FClist$NSL %>% 
  dplyr::filter(gene %in% nsl_hai_genes) %>%
  t() %>%
...
baseline_serostatus <- 
  baseline_serostatus %>%
  dplyr::filter(subid %in% nasal_fc$subid) %>%
  dplyr::select("subid", "n_prior_exposure") %>%
  left_join(nasal_fc, by= "subid")

baseline_serostatus$n_prior_exposure <- as.factor(baseline_serostatus$n_prior_exposure)
baseline_serostatus[ , 3:ncol(baseline_serostatus)] <- lapply(baseline_serostatus[ , 3:ncol(baseline_serostatus)], as.numeric)

lm_test <- lm(MX1 ~ as.numeric(n_prior_exposure), data = baseline_serostatus)
summary(lm_test)
...
baseline_serostatus %>%
  ggplot(aes(x = n_prior_exposure, y = MX1)) +
  geom_boxplot(outlier.shape = NA, colour = "darkblue") +
  geom_jitter(width = 0.1, size = 0.75, colour = "darkblue") +
  annotate("text", x = 1.75, y = 3, label = formatted_global_p, size = 4, hjust = 0) +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 12)) +
  xlab("Number of prior exposed strains") +
  ylab(expression(italic(MX1) ~ "expression (" * Log[2] * " fold change)"))

ggsave("./output/figs/fig4e.svg")
...
## Fig 4f
####
lm_test <- lm(HERC5 ~ as.numeric(n_prior_exposure), data = baseline_serostatus)
summary(lm_test)
...
baseline_serostatus %>%
  ggplot(aes(x = n_prior_exposure, y = HERC5)) +
  geom_boxplot(outlier.shape = NA, colour = "darkblue") +
  geom_jitter(width = 0.1, size = 0.75, colour = "darkblue") +
  annotate("text", x = 1.75, y = 5, label = formatted_global_p, size = 4, hjust = 0) +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
        axis.text = element_text(size = 12)) +
  xlab("Number of prior exposed strains") +
  ylab(expression(italic(HERC5) ~ "expression (" * Log[2] * " fold change)"))

ggsave("./output/figs/fig4f.svg")